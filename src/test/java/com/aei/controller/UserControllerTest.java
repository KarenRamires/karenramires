...
public class UserControllerTest extends BaseControllerTest {
	...
	public void view_user_profile() throws Exception {
		...
		this.mvc.perform(get("/user/profile").with(user(userDetails)).accept(MediaType.TEXT_HTML))
				.andExpect(status().isOk()).andExpect(content().contentType("text/html;charset=UTF-8"))
				.andExpect(view().name("/user/profile")).andExpect(model().attribute("user",
						allOf(hasProperty("id", is(USER_ID)), hasProperty("name", is(USER_NAME)))));
	}
	...
	public void save_user_profile() throws Exception {
		...
		this.mvc.perform(post("/user/save").with(user(userDetails)).with(csrf())
				.contentType(MediaType.APPLICATION_FORM_URLENCODED).param("id", USER_ID.toString())
				.param("name", USER_NAME).param("email", USER_EMAIL)).andExpect(view().name("redirect:/user/profile"))
				.andExpect(model().size(1)).andExpect(flash().attributeExists("message"))
